...
@section('content')
    <div class="pathway">
        <ul>
            <li><a href="http://127.0.0.1:8000/home" title="Trang chủ">Trang chủ \</a></li>
            <li>{{ $bookType->tenloai }}</li>
        </ul>
    </div>
    <div class="clear"></div>
    <h3 style="font-weight: bold;">
        @if (count($sach) < 1)
            Loại sách này chưa được cung cấp vui lòng chọn loại sách khác!@else{{ $bookType->tenloai }}
        @endif
    </h3>
    <div id="container">
                        ...
                        <div class="product">
                            <div class="image">
                                <div style="position: relative;">
                                    <a href="{{ route('chitietsanpham', $sachs->id) }}"><img
                                            src="{{ Storage::disk('book')->url($sachs->anhbia) }}" alt=""
                                            class=""></a>
                                    @if ($sachs->khuyenmai > 0)
                                        <span class="saleprice"
                                            style="background:url({{ url('public/image/saleprice.png') }}) no-repeat;">{{ $sachs->khuyenmai }}%</span>
                                    @endif
                                </div>
