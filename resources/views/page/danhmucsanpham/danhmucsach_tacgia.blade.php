...
@section('loaisach')
    @foreach ($ls as $loaisach)
        @if ($loaisach->madanhmuc == $id)
            <li><a href="{{ route('loaisach/', $loaisach->id) }}"
                    title="{{ $loaisach->tenloai }}">{{ $loaisach->tenloai }}</a>
            </li>
        @endif
    @endforeach
@endsection
@section('tacgia')
    @if (count($danhmucsach_id) > 0)
        @foreach ($tacgia as $sach_danhmuc_id_tacgia)
            <a href="{{ route('danhmuc/tacgia', [$danhmuc_id->id, $sach_danhmuc_id_tacgia['tacgia']]) }}"
                title="{{ $sach_danhmuc_id_tacgia['tacgia'] }}" class="checkboxs">
                <li class="checkbox">
                    <input type="radio" name="checktacgia"> {{ $sach_danhmuc_id_tacgia['tacgia'] }}<span
                        class="count">({{ $sach_danhmuc_id_tacgia['sl'] }})</span>
                </li>
            </a>
        @endforeach
    @else
                ...
                <div class="image">
                    <div style="position: relative;">
                        <a href="{{ route('chitietsanpham', $sach_danhmuc_id->id) }}"><img
                                src="{{ Storage::disk('book')->url($sach_danhmuc_id->anhbia) }}" alt=""
                                class=""></a>
                        @if ($sach_danhmuc_id->khuyenmai > 0)
                            <span class="saleprice"
                                style="background: url('{{ asset('/image/saleprice.png') }}') no-repeat;">{{ $sach_danhmuc_id->khuyenmai }}%</span>
                        @endif
                    </div>
                    <a href="{{ route('chitietsanpham', $sach_danhmuc_id->id) }}">
                        <div class="product_name" title="{{ $sach_danhmuc_id->tensach }}">{{ $sach_danhmuc_id->tensach }}
                        </div>
                    </a>
                    <div class="product_composer"><a href="{{ url('/tacgia/'.$sach_danhmuc_id->tacgia) }}">{{ $sach_danhmuc_id->tacgia }}</a></div>
                    <div class="prices">
        ...
        @foreach ($nxbs as $sach_nxb_id)
            @foreach ($nxb as $nhaxuatban)
                @if ($nhaxuatban->id == $sach_nxb_id['nxb'])
                    <a href="" title="Thái Hà">
                        <li class="checkbox">
                            <input type="radio" name="checknxb"> {{ $nhaxuatban->tennhaxuatban }}</i> <span
                                class="count">({{ $sach_nxb_id['sl'] }})</span>
